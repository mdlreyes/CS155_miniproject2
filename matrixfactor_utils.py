...
def get_err(U, V, Y, reg=0.0):
    ...
    for nTerm in range(len(Y)):
        ...
        errterm += (Y[nTerm][2] - np.dot(U[i,:],V.T[:,j]))**2.
...
def train_model(M, N, K, eta, reg, Y, eps=0.0001, max_epochs=300):
    ...
    while (nEpoch < max_epochs):
        ...
        for nTerm in range(len(Y)):
            ...
            U[i,:] -= grad_U(U[i,:], Y_shuffle[nTerm][2], V.T[:,j], reg, eta)
            V.T[:,j] -= grad_V(V.T[:,j], Y_shuffle[nTerm][2], U[i,:], reg, eta)

        # Calculate new error
        errNew = get_err(U,V,Y)
