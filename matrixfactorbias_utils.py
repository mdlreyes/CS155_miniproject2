...
# More complex matrix factorization (include bias terms a, b)
...
def grad_U(Ui, Yij, Vj, reg, eta, ai, bj):
    ...
    grad1 = reg*Ui 
    grad2 = 2.*Vj*(Yij-np.dot(Ui,Vj)-ai-bj)
...
def grad_V(Vj, Yij, Ui, reg, eta, ai, bj):
    ...
    """
    grad1 = reg*Vj
    grad2 = 2.*Ui*(Yij-np.dot(Ui,Vj)-ai-bj)
...
def train_model(M, N, K, eta, reg, Y, eps=0.0001, max_epochs=300):
    ...
    while (nEpoch < max_epochs):
        ...
        for nTerm in range(len(Y)):
            ...
            U[i,:] -= grad_U(U[i,:], Y_shuffle[nTerm][2], V.T[:,j], reg, eta, a[i], b[j])
            V.T[:,j] -= grad_V(V.T[:,j], Y_shuffle[nTerm][2], U[i,:], reg, eta, a[i], b[j])

            # Update bias terms
            a[i] -= grad_a(U[i,:], Y_shuffle[nTerm][2], V.T[:,j], reg, eta, a[i], b[j])
            b[j] -= grad_b(U[i,:], Y_shuffle[nTerm][2], V.T[:,j], reg, eta, a[i], b[j])

        # Calculate new error
        errNew = get_err(U,V,Y,a,b)
